...
@interface THParticleLayer : CALayer

@property (nonatomic, strong) UIBezierPath *particlePath;

@end

static NSArray *tempArr ;
static NSMutableArray *frameArrM;
static NSArray *tempLayerArr ;
static NSMutableArray *frameLayerArrM;

static id   layerContent ;

@implementation UIView (THExplode)
...
- (void)th_explodeWithDuration:(NSTimeInterval)duration callback:(ExplodeCompletion)callback
{
    ...
    tempArr = [self.subviews copy];
    tempLayerArr = [self.layer.sublayers copy];
    if (frameArrM == nil) {
        frameArrM = [NSMutableArray arrayWithCapacity:tempArr.count];
    }
    [frameArrM removeAllObjects];
    [tempArr enumerateObjectsUsingBlock:^(UIView  *_Nonnull subView, NSUInteger idx, BOOL * _Nonnull stop) {
        [frameArrM addObject:[NSValue valueWithCGRect:subView.frame]];
    }];
    
    if (frameLayerArrM == nil) {
        frameLayerArrM = [NSMutableArray arrayWithCapacity:tempLayerArr.count];
    }
    
    [frameLayerArrM removeAllObjects];
    [tempLayerArr enumerateObjectsUsingBlock:^(CALayer  *_Nonnull layer, NSUInteger idx, BOOL * _Nonnull stop) {
        [frameLayerArrM addObject:[NSValue valueWithCGRect:layer.frame]];
    }];
    
    [[self subviews] makeObjectsPerformSelector:@selector(removeFromSuperview)];
    [[self.layer sublayers] makeObjectsPerformSelector:@selector(removeFromSuperlayer)];
    layerContent = self.layer.contents ;
    ...
    for (int y = 0; y < fullRows; ++y)
    {
        for (int x = 0; x < fullColumns; ++x)
        {
            ...
            THParticleLayer *layer = [THParticleLayer layer];
    ...
    [sublayersArray enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        ...
        THParticleLayer *layer = (THParticleLayer *)obj;
...
{
    [self  th_explodeWithDuration:duration callback:nil];
}
...
- (void)animationDidStop:(CAAnimation *)theAnimation finished:(BOOL)flag
{
    ...
    THParticleLayer *layer = [theAnimation valueForKey:@"animationLayer"];
    ...
    if (layer)
    {
        ...
        {
            self.layer.contents = layerContent;
            [tempLayerArr enumerateObjectsUsingBlock:^(CALayer  *_Nonnull subLayer, NSUInteger idx, BOOL * _Nonnull stop) {
                subLayer.frame = [frameLayerArrM[idx] CGRectValue];
                [self.layer addSublayer:subLayer];
            }];
            
            
            [tempArr enumerateObjectsUsingBlock:^(UIView  *_Nonnull subView, NSUInteger idx, BOOL * _Nonnull stop) {
                subView.frame = [frameArrM[idx] CGRectValue];
...
@implementation THParticleLayer
